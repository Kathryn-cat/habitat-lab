...
from habitat.tasks.expl.expl_sim import ExplSim
from habitat.tasks.expl.utils import rearrange_collision
...
class ArmAction(SimulatorTaskAction):
    ...
    def __init__(self, *args, config, sim: ExplSim, **kwargs):
...
class MagicGraspAction(SimulatorTaskAction):
    ...
    def _grasp(self):
        ...
        if len(scene_obj_pos) != 0:
            # TODO: change - Get the target the EE is closest to.
...
class BaseVelAction(SimulatorTaskAction):
    ...
    def _set_robo_state(self, robot_id, sim: ExplSim, set_dat):
